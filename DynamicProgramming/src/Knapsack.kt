 ...
 * Exponential Time
 */
fun recursiveKnapsack(w: IntArray, p: IntArray, capacity: Int): Int {
...
private fun helper(
    ...
    if (capacity - w[index] >= 0) {
        op2 = helper(index + 1, w, p, capacity - w[index]) + p[index]
    }
 ...
 * W * C time and space
 */
fun recursiveKnapsackWithMemo(w: IntArray, p: IntArray, capacity: Int): Int {
...
private fun helper(
    ...
    var op2 = 0
    if (capacity - w[index] >= 0) {
        op2 = helper(index + 1, w, p, capacity - w[index], cache) + p[index]
    }
 ...
 * W * C time and space
 */
fun knapsackWithTab(w: IntArray, p: IntArray, capacity: Int): Int {
    ...
    for (index in 1..cache.lastIndex) {
        for (cap in 1..cache[0].lastIndex) {
            ...
            var profit2 = 0
            if (cap - w[index - 1] >= 0) {
                profit2 = cache[index - 1][cap - w[index - 1]] + p[index - 1]
            }
 ...
 */
fun knapsackWithTabLinearSpace(w: IntArray, p: IntArray, capacity: Int): Int {
